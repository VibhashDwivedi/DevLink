...
const ProfilePost = () => {
  ...
  const fetchFollowData = async () => {
    const res = await fetch(
      "https://devlink-project.onrender.com/follow/getall",
      {
        method: "GET",
        headers: {
  ...
  const fetchUserLikes = async () => {
    const res = await fetch(
      "https://devlink-project.onrender.com/likes/getall"
    );
    console.log(res.status);
  ...
  const deletepost = async (id) => {
    ...
    if (c === true) {
      const res = await fetch(
        "https://devlink-project.onrender.com/post/delete/" + id,
        { method: "DELETE" }
      );
      if (res.status === 200) {
        fetchUserData1();
  ...
  const displayPost = () => {
    ...
    return post.map((posts) => {
      return (
        <div
          className="card shadow-lg mt-4 p-2"
        ...
        >
          ...
          <div className=" text-black mx-2 pb-2 fw-light  ">
            {posts.content}
          </div>
          <div className=" text-black mx-2 pb-2 fw-light  ">
            <i className="fa-solid fa-heart " style={{ color: "red" }}></i>{" "}
            {countLikes(posts._id)}
          </div>
  ...
  const deleteuser = async (id) => {
    ...
    if (c === true) {
      const res = await fetch(
        "https://devlink-project.onrender.com/user/delete/" + id,
        { method: "DELETE" }
      );
      if (res.status === 200) {
        fetchUserData1();
  ...
  const displayFollowers = () => {
    return myfollowers.map((follow) => {
      return (
        <div
          className="card shadow-lg mt-4 p-2"
          style={{ border: "none", backgroundColor: "wheat" }}
        >
          <div className="d-flex">
            <div className=" text-black fw-bold mx-2  fs-4">
              {follow.username}
            </div>

            <div className=" text-black mx-3 pb-2 fw-light  ">
              {follow.profile}
            </div>
          </div>
        </div>
    ...
    if (currentUser.avatar === "") {
      ...
      return (
        <img
          ...
          src={"https://devlink-project.onrender.com/" + currentUser.avatar}
          alt=""
        />
      );
    }
  ...
  const displayprofile2 = () => {
    if (currentUser.avatar === "") {
      return (
        <i
          className="fa-solid fa-user fa-xl mx-2 "
          style={{ color: "#8c8c8c" }}
        ></i>
      );
    } else {
        ...
        <header className="header-bar2 mb-3">
          <div className="container d-flex flex-column flex-md-row align-items-center p-3">
            <h3 className="my-0 mr-md-auto fw-bold ">
              <div className="text-white text-decoration-none">DevLink</div>
            </h3>
            <div className="flex-row my-3 my-md-0 ms-auto">
              ...
              <Link
                className="btn btn-sm btn-success mr-2 mx-3"
                to="/createpost"
              >
                Create Post
              </Link>
          ...
          <div className="card-header">
            <div className="d-flex">
              ...
              <div className="d-flex ms-auto">
                <Link to="/edituser">
                  <button className=" btn h-75 btn-primary">Edit</button>
                </Link>

                <button
                  className=" ms-2 btn h-75 btn-danger"
                  onClick={() => {
            ...
            <p
              ...
              style={{ marginTop: "-20px", marginLeft: "58px" }}
            >
              {currentUser.profile}
            </p>

            <div className="profile-nav nav nav-tabs pt-2 mb-4">
              <button className="btn btn-dark">Posts : {countPost()}</button>
